 ...
 */
public class TuermeVonHanoiAnleitung extends JPanel implements Runnable, ActionListener {
    ...
    public static void main(String[] args) {
        JFrame window = new JFrame("Tuerme von Hanoi Anleitung");
        window.setContentPane(new TuermeVonHanoiAnleitung());
    ...
    */
     ...
     */
    public TuermeVonHanoiAnleitung() {
        ...
        delaySpinner = new JSpinner(new SpinnerNumberModel(80, 1, Integer.MAX_VALUE, 5));
        delaySpinner.addChangeListener(e -> waitTime = ((Integer) delaySpinner.getValue()));
        delaySpinner.setEnabled(true);
        buttonBar.add(delaySpinner);
        amountSpinner = new JSpinner(new SpinnerNumberModel(5, 1, 10, 1));
        amountSpinner.addChangeListener(e -> {
            disks = ((Integer) amountSpinner.getValue());
            status = RESTART;
        });
        amountSpinner.setEnabled(true);
        buttonBar.add(amountSpinner);
        stepsLabel = new JLabel();
        buttonBar.add(stepsLabel);
    ...
    synchronized private void setUpProblem() {
        ...
        display.repaint();
        stepsLabel.setText(String.format("Steps: %d", steps));
    }
    ...
    synchronized private void drawCurrentFrame(Graphics g) {
        ...
        String start = "start";
        g.drawChars(start.toCharArray(), 0, start.length(), 10, 30);
        g.fillRect(150, 128, 130, 5);
        g.fillRect(290, 128, 130, 5);
        String target = "target";
        g.drawChars(target.toCharArray(), 0, target.length(), 290, 30);
